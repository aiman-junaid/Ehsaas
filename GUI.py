...
def MapClick(i,latt,longi):
    ...
    print(i.Name)
    folium.Marker([longi, latt],popup=(i.Name)).add_to(my_map1)
    coordinates = []
    coordinates.append([latt, longi])
    coordinates.append([round(lat, 2), round(lng, 2)])
    my_PolyLine = folium.PolyLine(locations=coordinates, weight=5)
    my_map1.add_child(my_PolyLine)
    my_map1.save("my_map"+str(counter)+".html")
    webbrowser.get("C:/Program Files (x86)/Google/Chrome/Application/chrome.exe %s").open('my_map'+str(counter)+".html",new=2)
    my_PolyLine.render()
...
def data(Select, entity):
    ...
    if AreaChoosen.current() == - 1:
        result = tree.Search(entity,AreaChoosen['values'][AreaChoosen.current()],False)
        for i in result :
        ...
        result = tree.Search(entity,AreaChoosen['values'][AreaChoosen.current()],True)
        for i in result :
